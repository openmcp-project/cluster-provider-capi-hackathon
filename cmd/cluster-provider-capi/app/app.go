...
func (o *SharedOptions) AddPersistentFlags(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().StringVar(&o.Environment, "environment", "", "Environment name. Must be globally unique.")
	cmd.PersistentFlags().StringVar(&o.ProviderName, "provider-name", "", "Name of the ClusterProvider resource.")
	cmd.PersistentFlags().BoolVar(&o.DryRun, "dry-run", false, "Abort after evaluating flags without running.")
}
